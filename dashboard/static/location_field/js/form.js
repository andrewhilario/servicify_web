    ...
    $.locationField = function(options) {
        var LocationField = {
            ...
            render: function() {
                ...
                this.$getloc_id = $('#' + this.options.getloc_id);
                ...
                this.loadAll(function(){
                    ...
                    self._watchGetLocButton(self.options.getloc_id, map, marker);
                });
            },
                ...
                if (this.options.searchProvider === 'google') {
                    ...
                    googleGeocodeProvider.GetLocations(address, function(data) {
                        if (data.length > 0) {
                            ...
                            localStorage.setItem("locData", JSON.stringify(data[0]));
            ...
            _getMarker: function(map, center) {
                ...
                map.on('click', function(e){
                    ...
                    var googleGeocodeProvider = new L.GeoSearch.Provider.Google();
                    googleGeocodeProvider.GetLocations(
                      e.latlng.lat + "," + e.latlng.lng,
                      function (data) {
                        if (data.length > 0) {
                          if (basedFields) {
                            $(basedFields.selector).val(data[0].Label);
                          }
                          localStorage.setItem("locData", JSON.stringify(data[0]));
            ...
            _watchGetLocButton: function(locbtn, map, marker) {
                // get location button
                let basedFields = this.options.basedFields
                $("#"+locbtn).on("click", function(e) {
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                          (position) => {
                            var googleGeocodeProvider = new L.GeoSearch.Provider.Google();
                            googleGeocodeProvider.GetLocations(position.coords.latitude + "," + position.coords.longitude, function(data) {
                                if (data.length > 0) {
                                    if (basedFields) {
                                        $(basedFields.selector).val(data[0].Label);
                                    }
                                    ...
                                    // TODO: Store sublocality, locality address component to the backend.
                                }
                            });
                          },
                          (e) => {
                            if (e.code === 1) {
                                window.alert(e.code, e.message, "Please allow location permissions.")
                            }
                          }
                        );
                    } else {
                        window.alert("cant open")
                    }
                });
            },
